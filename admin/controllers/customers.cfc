...
<cfcomponent extends="controller" output="false" persistent="false" accessors="true">
	...
	<cffunction name="contacts" returntype="any" output="false">
		...
		<cfelse>
			...
			<cfquery name="Session.getClientContacts" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
				Select p_inv_Positions.PositionTitle, p_inv_CustomerContacts.Customer_ID, p_inv_CustomerContacts.TContent_ID, p_inv_CustomerContacts.Contact_FirstName, p_inv_CustomerContacts.Contact_LastName, p_inv_CustomerContacts.Contact_Email,
					p_inv_CustomerContacts.Contact_PhoneNumber, p_inv_CustomerContacts.Contact_PhoneExtension
				From p_inv_CustomerContacts INNER JOIN p_inv_Positions ON p_inv_Positions.TContent_ID = p_inv_CustomerContacts.PositionHeld
				Where p_inv_CustomerContacts.Site_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rc.$.siteConfig('siteID')#"> and
					p_inv_CustomerContacts.Customer_ID = <cfqueryparam value="#URL.RecNo#" cfsqltype="cf_sql_integer">
	...
	<cffunction name="addclientcontacts" returntype="any" output="false">
		...
		<cfif not isDefined("FORM.formSubmit")>
			<cflock timeout="60" scope="Session" type="Exclusive">
				...
				<cfquery name="Session.getSelectedClient" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
					Select TContent_ID, Site_ID, BusinessName, PhysicalAddress, PhysicalCity, PhysicalState, PhysicalZipCode
					From p_inv_Customers
					Where Site_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rc.$.siteConfig('siteID')#"> and
						TContent_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.RecNo#">
				</cfquery>
				<cfquery name="Session.getPositions" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
					Select TContent_ID, Site_ID, PositionTitle, Active
		...
		<cfelseif isDefined("FORM.formSubmit")>
			...
			<cfif FORM.ContactPosition EQ "----">
				<cflock timeout="60" scope="SESSION" type="Exclusive">
					<cfset Session.FormErrors = #ArrayNew()#>
					<cfscript>
						address = {property="Active",message="Please Select the current position title for this contact"};
						arrayAppend(Session.FormErrors, address);
					</cfscript>
				</cflock>
				<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:customers.addclientcontacts&PerformAction=MissingInfo&SiteID=#rc.$.siteConfig('siteID')#&RecNo=#FORM.CustomerID#" addtoken="true">
			</cfif>
			...
			<cfif not isValid("email", FORM.ContactEmail)>
				<cflock timeout="60" scope="SESSION" type="Exclusive">
					<cfset Session.FormErrors = #ArrayNew()#>
					<cfscript>
						address = {property="Active",message="Please correct the Email Address so that it is in proper format."};
						arrayAppend(Session.FormErrors, address);
					</cfscript>
				</cflock>
				<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:customers.addclientcontacts&PerformAction=MissingInfo&SiteID=#rc.$.siteConfig('siteID')#&RecNo=#FORM.CustomerID#" addtoken="true">
			...
			<cfquery name="insertNewPaymentTerms" result="insertNewPaymentTerms" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
				Insert into p_inv_CustomerContacts(Site_ID, Customer_ID, Contact_FirstName, Contact_LastName, Contact_Email, Contact_PhoneNumber, Contact_PhoneExtension, PositionHeld, dateCreated, lastUpdatedBy)
				Values(
				<cfqueryparam value="#rc.$.siteConfig('siteID')#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam cfsqltype="cf_sql_integer" value="#URL.RecNo#">,
				<cfqueryparam value="#FORM.ContactFirstName#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam value="#FORM.ContactLastName#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam value="#FORM.ContactEmail#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam value="#FORM.ContactPhone#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam value="#FORM.ContactPhoneExtension#" cfsqltype="cf_sql_varchar">,
				<cfqueryparam value="#FORM.ContactPosition#" cfsqltype="cf_sql_integer" >,
				<cfqueryparam value="#Now()#" cfsqltype="cf_sql_timestamp">,
				<cfqueryparam value="#rc.$.currentUser('userName')#" cfsqltype="cf_sql_varchar">
				)
			</cfquery>
			<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:customers.contacts&UserAction=AddedContact&SiteID=#rc.$.siteConfig('siteID')#&Successful=true&RecNo=#URL.RecNo#" addtoken="false">


		</cfif>
	</cffunction>

	<cffunction name="updateclientcontacts" returntype="any" output="false">
		<cfargument name="rc" required="true" type="struct" default="#StructNew()#">

		<cfif isDefined("URL.UserAction")>
			<cfswitch expression="#URL.UserAction#">
				<cfcase value="DeleteContact">
					<cfquery name="DeleteContact" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
						Delete from p_inv_CustomerContacts
						Where TContent_ID = <cfqueryparam value="#URL.RecNo#" cfsqltype="cf_sql_integer">
					</cfquery>
					<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:customers.contacts&UserAction=DeletedContact&SiteID=#rc.$.siteConfig('siteID')#&Successful=true&RecNo=#URL.ClientID#" addtoken="false">
				</cfcase>
			</cfswitch>
		</cfif>
		<cfif not isDefined("FORM.formSubmit")>
			<cflock timeout="60" scope="Session" type="Exclusive">
				...
				<cfquery name="Session.getSelectedClient" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
					...
					From p_inv_Customers
					Where Site_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rc.$.siteConfig('siteID')#"> and
						TContent_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.ClientID#">
				</cfquery>

				<cfquery name="Session.getSelectedContact" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
					Select TContent_ID, Site_ID, Contact_FirstName, Contact_LastName, Contact_Email, Contact_PhoneNumber, Contact_PhoneExtension, PositionHeld
					From p_inv_CustomerContacts
					Where Site_ID = <cfqueryparam cfsqltype="cf_sql_varchar" value="#rc.$.siteConfig('siteID')#"> and
						TContent_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.RecNo#">
				</cfquery>
		...
		<cfelseif isDefined("FORM.formSubmit")>
			...
			<cftry>
			<cfquery name="updateClientContact" Datasource="#rc.$.globalConfig('datasource')#" username="#rc.$.globalConfig('dbusername')#" password="#rc.$.globalConfig('dbpassword')#">
				Update p_inv_CustomerContacts
				Set Contact_FirstName = <cfqueryparam value="#FORM.ContactFirstName#" cfsqltype="cf_sql_varchar">,
					Contact_LastName = <cfqueryparam value="#FORM.ContactLastName#" cfsqltype="cf_sql_varchar">,
					Contact_Email = <cfqueryparam value="#FORM.ContactEmail#" cfsqltype="cf_sql_varchar">,
					Contact_PhoneNumber = <cfqueryparam value="#FORM.ContactPhone#" cfsqltype="cf_sql_varchar">,
					Contact_PhoneExtension = <cfqueryparam value="#FORM.ContactPhoneExtension#" cfsqltype="cf_sql_varchar">,
					PositionHeld = <cfqueryparam value="#FORM.ContactPosition#" cfsqltype="cf_sql_integer">,
					LastUpdated = <cfqueryparam value="#Now()#" cfsqltype="cf_sql_timestamp">,
					LastUpdatedBy = <cfqueryparam value="#rc.$.currentUser('userName')#" cfsqltype="cf_sql_varchar">
				Where TContent_ID = <cfqueryparam cfsqltype="cf_sql_integer" value="#URL.RecNo#">
			</cfquery>
				<cfcatch type="any">
					<cfdump var="#CFCATCH#"><cfabort>
				</cfcatch>
			</cftry>
			<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:customers.contacts&UserAction=UpdatedContact&SiteID=#rc.$.siteConfig('siteID')#&Successful=true&RecNo=#URL.ClientID#" addtoken="false">
